<header>
      @php
         $headerSection = App\Models\ForntendSetting::where('key', 'heder-menu-setting')->first();
         $sectionData = $headerSection ? json_decode($headerSection->value, true) : null;
      @endphp

    {{--TOP BAR START--}}
    <div class="top-header bg-primary #d-md-block">
                ...
                <ul class="top-header-left list-inline d-flex align-items-center gap-3 m-0">
                   <li>
                        @php
                           ...
                           $appsetting = json_decode($generalsetting->value);

                        @endphp
                      <a class="text-white d-flex align-items-center" href="tel:{{ $appsetting->helpline_number }}">
                         ...
                         <span>{{ $appsetting->helpline_number }}</span>
                      </a>
                   </li>
                </ul>
             </div>
    ...
    {{--HEADER ROW (LOGO, MENU, LOGIN/REGISTER BUTTON) START--}}
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                   <ul class="navbar-nav align-items-center ms-auto mb-2 mb-xl-0 p-0">
                     ...
                     @if($sectionData['enable_darknight_mode'] == 1)
                      <li class="nav-item theme-scheme-dropdown dropdown iq-dropdown d-none">
                     ...
                     @endif
                      ...
                      @if(empty(auth()->user()) || auth()->user()->user_type !== 'user')
                        <li class="ms-sm-3 ms-2">
                           <a href="{{route('user.register')}}" class="btn btn-outline-primary font-weight-600" role="button">
                               {{ __('auth.register') }}
                           </a>
                        </li>
                       <li class="ms-sm-3 ms-2">
                           <a href="{{route('user.login')}}" class="btn btn-primary font-weight-600" role="button">
                               {{ __('auth.login') }}
                           </a>
                       </li>
                      @else
