...
const fetchService = async () => {
   ...
   const defaultEarningType = async () => {
    try {
        ...
        const data = await response.json();
        const earningType = data && data.general_settings.earning_type ? data.general_settings.earning_type : 'commission';
        return earningType;
    } catch (error) {
  ...
  try {
    ...
    if (settings) {
      ...
      if(props.countRef != ''){
        let element = document.getElementById(props.countRef);
        element.innerHTML = '+' +serviceDetails.value.length;
      }
    }
  } catch (error) {
