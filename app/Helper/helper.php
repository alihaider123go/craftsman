...
function saveBookingActivity($data)
{
    ...
    switch ($data['activity_type'])
    {
        ...
        case "update_booking_status":
                ...
                $sendTo = removeArrayValue($role, ['admin', 'provider' , 'handyman' , 'user','demo_admin']);
            break;
        case "cancel_booking":
            $status = \App\Models\BookingStatus::bookingStatus($data['booking']->status);
            $old_status = \App\Models\BookingStatus::bookingStatus($data['booking']->old_status);
...
function getPriceFormat($price){
    ...
    $currencyId = $sitesetupdata->default_currency;
    $currency_position = $sitesetupdata->currency_position;
    $afterdecimalpoint = $sitesetupdata->digitafter_decimal_point;
...
function default_earning_type(){
    ...
    $earningtype = $gettype->value ? $gettype->value : 'commission';
    return $earningtype;
}
...
function getHandymanServiceTimeSlotByDate($provider_id, $handyman_id,$date)
{
    $sitesetup = App\Models\Setting::where('type','site-setup')->where('key', 'site-setup')->first();
    $admin = json_decode($sitesetup->value);
    date_default_timezone_set($admin->time_zone ?? 'UTC');
    $days = ['Monday'=>'mon', 'Tuesday'=>'tue','Wednesday'=>'wed',"Thursday"=> 'thu','Friday'=> 'fri','Saturday'=> 'sat','Sunday'=> 'sun'];
    $reqDate = \Carbon\Carbon::createFromFormat('Y-m-d',$date);
    
    $current_time = \Carbon\Carbon::now();
    $time = $current_time->toTimeString();
    $current_day = $days[$reqDate->format('l')];
    
    $handyman_count = \App\Models\User::where('provider_id', $provider_id)->where('is_available', 1)->count() + 1;
    
    $handymanSlots = \App\Models\HandymanSlotMapping::where('handyman_id', $handyman_id)
        ->where('days', $current_day)
        ->orderBy('start_at', 'asc')
        ->get();
    
    $bookings = \App\Models\Booking::where('provider_id', $provider_id)->get();
    
    $specificHandymanBooking = [];
    foreach ($bookings as $key => $booking) {
        
        $bookingDate = \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $booking->date);
        
        if(count($booking->handymanAdded) > 0 && $booking->handymanAdded[0]->handyman_id == $handyman_id && $reqDate->format('Y-m-d')==$bookingDate->format('Y-m-d')){
            $booking->handyman_id = $handyman_id;
            $specificHandymanBooking[]=$booking;
        }
    }
    
    $specificHandymanBookingCollection = collect($specificHandymanBooking);

    $current_date = $current_time->format('Y-m-d');    
    if ($current_date == $reqDate->format('Y-m-d')) {
        $handymanSlots = $handymanSlots->where('start_at', '>', $time);
    }

    $filteredSlots = $handymanSlots->pluck('start_at')->toArray();
    if(count($specificHandymanBookingCollection)>0){
        $filteredSlots = array_diff($filteredSlots, $specificHandymanBookingCollection->pluck('booking_slot')->toArray());
    }
    
    $formatedArray = [];
    foreach($filteredSlots as $object)
    {
...
function getHandymanServiceNextAvailableSlotByDate($provider_id, $handyman_id,$date){
    $baseDate = \Carbon\Carbon::createFromFormat('Y-m-d',$date);
    for ($i=1; $i < 15; $i++) {
        $baseDate = $baseDate->addDays(1);
        $slots = getHandymanServiceTimeSlotByDate($provider_id, $handyman_id,$baseDate->format('Y-m-d'));
        if(count($slots)>0){
...
function getHandymanServiceTimeSlot($provider_id, $handyman_id)
{
    $sitesetup = App\Models\Setting::where('type','site-setup')->where('key', 'site-setup')->first();
    $admin = json_decode($sitesetup->value);
    date_default_timezone_set($admin->time_zone ?? 'UTC');
    ...
    foreach ($days as $value) {
        $slot = $handymanSlots->where('days', $value);
...
function nearestAvailableSlotHandyman($provider_id, $handymanList) {
    $current_day = strtolower(date('D'));
    ...
    $todayAvailableHandyman = [];
    $todayNotAvailableHandyman = [];

    $last_handyman_available_index = $current_day_index;
    for ($j=0; $j < count($handymanList); $j++) { 
        $singlehandymanserviceslots = getHandymanServiceTimeSlot($provider_id, $handymanList[$j]->id);

        foreach ($singlehandymanserviceslots as $slot => $value) {
            if($current_day == $value['day'] && count($value['slot']) > 0) {
                array_push($todayAvailableHandyman, $handymanList[$j]);
            }
            if (count($todayAvailableHandyman) == 0)
            {
                for ($k=$current_day_index+1; $k < count($days); $k++) {
                    if($days[$k] == $value['day'] && count($value['slot']) > 0) {
                        if($k <= $last_handyman_available_index )
                        {
                            array_push($todayAvailableHandyman, $handymanList[$j]);
                        }
                        else
                        {
                            $last_handyman_available_index = $k;
                            array_push($todayNotAvailableHandyman, $handymanList[$j]);
                        }
                    }
                }
            }
            // else if ($current_day == $value['day'] && count($value['slot']) == 0)
            // {
            //     array_push($todayNotAvailableHandyman, $handymanList[$j]);
            // }
        }
    ...
    // return $todayAvailableHandyman;
    // return uniqueValues($todayAvailableHandyman);
    // $todayAvailableHandyman = uniqueValues($todayAvailableHandyman);
    // $todayNotAvailableHandyman = uniqueValues($todayNotAvailableHandyman);
    // return $todayAvailableHandyman;
    $providerhandymanlist = array_merge($todayAvailableHandyman, $todayNotAvailableHandyman);
    // return $providerhandymanlist;
    return collect($providerhandymanlist)->unique('id');

    // return uniqueValues($providerhandymanlist);
}
function uniqueValues($array_list) {
    $uniques = array();
    foreach ($array_list as $c) {
        $uniques[$c->id] = $c; // Get unique country by code.
    }
    return $uniques;
}
...
function providerpayout_rezopayX($data){
    ...
    if($rezorpay_data){
        ...
        // $url = "https://api.razorpay.com/v1/payouts"; // Replace with your Razorpayx Url
