...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            $start = (!empty($_GET["start"])) ? date('Y-m-d H:i:s', strtotime($_GET["start"])) : ('');
            $end = (!empty($_GET["end"])) ? date('Y-m-d H:i:s', strtotime($_GET["end"])) : ('');
            ...
            if ($user->hasRole('handyman') || $user->hasRole('provider')) {
                $data =  Booking::myBooking()->whereDate('date', '>=', $start)->whereDate('date',   '<=', $end)->with('service')->get();
            }
    ...
    public function removeFile(Request $request)
    {
        ...
        switch ($type) {
            ...
            case 'section5_attachment':
                ...
                break;
            case 'main_image':
                $data = ForntendSetting::find($request->id);
