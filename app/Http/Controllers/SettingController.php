...
class SettingController extends Controller
{
    ...
    public function layoutPage(Request $request)
    {
        ...
        switch ($page) {
            ...
            //     $auth_user = authSession();
            //     $assets = ['datatable'];
            //     $data = view('setting.' . $page, compact('settings', 'page','pageTitle','auth_user','assets','filter'))->render();
            //     break;
            // case 'taxes':
            //     $filter = [
            //         'status' => $request->status,
            ...
            case 'role-permission-setup':
                ...
                break;
            case 'time_slot':
                // $data  = view('setting.' . $page, compact('user_data', 'pageTitle'))->render();
                $data  = view('setting.' . $page, compact('user_data', 'page','slotsArray', 'pageTitle', 'activeDay', 'handyman_id', 'activeSlots'))->render();
                break;
            case 'provider_requests':
                $pageTitle = trans('messages.provider_requests');
                // $data  = view('setting.' . $page, compact('user_data', 'pageTitle'))->render();
                $data  = view('setting.' . $page, compact('user_data', 'page', 'pageTitle', 'handyman_id'))->render();
                break;
            case 'password_form':
    ...
    Public function provider_requests()
    {
        ...
        $pageTitle = trans('messages.provider_requests');
        $page = 'provider_requests';
        return view('setting.provider_requests', compact('user_data', 'page', 'pageTitle', 'handyman_id'));
    }

    Public function handyman_provider_list(DataTables $datatable,Request $request)
    {
        ...
        ->editColumn('display_name', function($provider) {
            return ($provider->status == 1) ? $provider->display_name : '-';
        })
        ->filterColumn('display_name', function ($query, $keyword) {
            $query->where('display_name', 'like', '%' . $keyword . '%');
        })
        ->editColumn('email', function($provider) {
            return ($provider->status == 1) ? $provider->email : '-';
        })
        ->filterColumn('email', function ($query, $keyword) {
            $query->where('email', 'like', '%' . $keyword . '%');
        })
        ->addColumn('action', function($query) {
            $status = '<a class="btn-sm text-white btn-success"  href='.route('handyman.request', ['null', $query->id, 0]).'>'.__('messages.send_request').'</a>';
            return $status;
        })
        ->addIndexColumn()
        ->toJson();
